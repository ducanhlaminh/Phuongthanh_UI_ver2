    ...
    if (actionType === actionTpyeLogin) {
      ...
      if (confirmPassword === password && password.length >= 6) {
        const handlCheckTokenAndCPW = async () => {
          let resone = await ApiChangePassword.verifyForgetEmail({
            email,
            token: urlParams.get("token"),
          });
          let tokenVerifyEmailSuccess = resone.tokenVerifyEmailSuccess;
          let tokenTemp = resone.token;
          let res = await ApiChangePassword.updatePasswordNoLocalToken(
            tokenTemp,
            password,
        ...
        <div className="h-full w-full flex items-center justify-center">
          <div className="z-10 flex flex-col w-full justify-center items-center bg-transparent">
            <div className="box-login w-11/12 max-h-[80%] md:w-[500px]  bg-white rounded-md p-5 ">
              ...
              {actionType === actionTypeChangePassword && (
                <h3 className="font-medium text-[16px] lg:text-[26px] md:text-[18px] md:font-semibold text-center ">
                  {email}
                </h3>
              )}
              <div>
                ...
                {actionType === actionTpyeSigup && (
                 ...
                 {actionType !== actionTypeForgotPassword && (
                  <InputFieldWithValidatePassword
                    lable={"Mật khẩu"}
                    PLarge={true}
                    value={password}
              ...
              <div className="flex flex-col gap-5 mt-[12px]">
                ...
                <div
                  ...
                  className={`text-[16px] text-primary cursor-pointer ${
                    actionType === actionTypeChangePassword ? "hidden" : "block"
                  }`}
                >
                  {actionType !== actionTypeForgotPassword
                    ? actionTypeForgotPassword
                    : "Đăng nhập."}
                </div>
              </div>
            </div>
